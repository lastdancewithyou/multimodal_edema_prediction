...
class Model(pl.LightningModule):
    def __init__(self, d_static_num, d_time_series_num, d_target, lr=3.e-4, weight_decay=1.e-1, glu=False,
            ...
            transformer_dropout=0., pos_frac=None, freeze_encoder=False, seed=42, save_representation=None,
        ...
        metric_kw = dict(task=task) if task == 'binary' else dict(task=task, num_classes=num_classes)

        self.train_auroc = torchmetrics.AUROC(**metric_kw)
        self.val_auroc   = torchmetrics.AUROC(**metric_kw)
        self.train_ap    = torchmetrics.AveragePrecision(**metric_kw)
        self.val_auprc      = torchmetrics.AveragePrecision(**metric_kw)
        self.test_auroc  = torchmetrics.AUROC(**metric_kw)
        self.test_auprc  = torchmetrics.AveragePrecision(**metric_kw)
    ...
    def configure_optimizers(self):
        optimizers = [torch.optim.AdamW(self.parameters(), lr=self.lr, weight_decay=self.weight_decay)]
        return optimizers
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.pretrain:
            ...
            self.val_auprc.update(y_hat, y.to(int).to(self.device))

        if not self.pretrain:
            self.log('val_auprc', self.val_auprc, on_epoch=True, sync_dist=True, rank_zero_only=True)
    ...
    def on_validation_epoch_end(self):
        if not self.pretrain:
            ...
            print(f'[epoch {ep:>3d}] val_auroc={self.val_auroc.compute():.4f}  '
            f'val_auprc={self.val_auprc.compute():.4f}')
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.test_auprc.update(y_hat, y.to(int).to(self.device))
        self.log('test_auprc', self.test_auprc, on_epoch=True, sync_dist=True, rank_zero_only=True)

        return loss, self.test_auroc, self.test_auprc
